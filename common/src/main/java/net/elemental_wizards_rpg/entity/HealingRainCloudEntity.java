...
public class HealingRainCloudEntity extends Entity implements SpellEntity.Spawned {
    ...
    private static final TrackedData<Integer> OWNER_ID_TRACKER = DataTracker.registerData(HealingRainCloudEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private void findLowestHealthTarget() {
        ...
        for (PlayerEntity player : nearbyPlayers) {
            if (player == owner || !isProtected(owner, player)) {
    ...
    private void healEntities() {
        ...
        for (Entity entity : entities) {
            ...
            RegistryEntry<Spell> spellImpact = SpellRegistry.from(owner.getWorld()).getEntry(Identifier.of(MOD_ID, "aqua_healing_rain_impact")).get();
