...
public class StormDraftEntity extends Entity implements SpellEntity.Spawned {
    ...
    private static final TrackedData<Integer> OWNER_ID_TRACKER = DataTracker.registerData(StormDraftEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private void checkCollision() {
        ...
        Box searchBox = this.getBoundingBox().expand(4.0);
        var entities = this.getWorld().getOtherEntities(this, searchBox);

        boolean hitEntity = false;
        for (Entity entity : entities) {
            RegistryEntry<Spell> spellImpact = SpellRegistry.from(owner.getWorld()).getEntry(Identifier.of(MOD_ID, "wind_gust")).get();
